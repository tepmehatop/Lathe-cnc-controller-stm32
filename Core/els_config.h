 ...
 *  MIN_AFEED=15 (0.15 мм/мин): при меньших значениях step_hz < 1 Hz → мотор стоит.
 *  Формула: step_hz = aFeed_100 * STEPS_PER_MM_Y / 6000 ≥ 1 → aFeed_100 ≥ 6000/400 = 15. */
#define MIN_AFEED               15
...
// ============================================================================
// ПАРАМЕТРЫ РУЧНОГО ЭНКОДЕРА (HandCoder)
// ============================================================================

/** Линий на оборот ручного энкодера. */
#define HAND_ENC_PPR            100

/** Скорость мотора при маштабе ×1 (Гц).
 *  1 тик = 4 шага для Y. При 100 тик/сек → 400 шагов/сек = 1 мм/сек (SCREW_Y=2мм).
 *  2000 Гц даёт запас до 500 тик/сек = 5 об/сек. */
#define HAND_HC_SPEED_1         2000

/** Скорость мотора при масштабе ×10 (Гц).
 *  1 тик = 40 шагов. При 200 тик/сек → 8000 шагов/сек.
 *  15000 Гц даёт запас. */
#define HAND_HC_SPEED_10        15000

// ============================================================================
// АЦП — переменник подачи
// ============================================================================

/** Пин переменника подачи (ADC12_IN4, PA4).
 *  Свободный аналоговый вход — не пересекается с другой периферией.
 *  Если переменник не подключён — закоротить на GND или 3.3В,
 *  либо отключить USE_ADC_FEED. */
#define USE_ADC_FEED            0       // 1 = включить, 0 = отключить
#define ADC_FEED_PIN            PA4     // свободный AIN на STM32F407

// ============================================================================
// ПАРАМЕТРЫ РУЧНОГО УПРАВЛЕНИЯ (джойстик)
// ============================================================================

/** Частота шагов при быстром ручном ходу (джойстик без кнопки RAPID).
 *  При SCREW_Y=2мм, MICROSTEP=4, STEPS/MM=400: 3000/400 = 7.5 мм/с = 450 мм/мин.
 *  Регулируйте по скорости вашего станка. */
#define RAPID_TRAVERSE_HZ       3000
